...
var updateDog = function(req, res) {
	...
	//Check if there is a query parameter for name
    if(!req.body.name) {
		return res.json({error: req.body.name});
        //return res.json({error: "Name is required to perform a search"});
    }
    
	//Call the dogs find by name function
...
var updateLast = function(req, res) {
	...
	//lastAdded.age = req.body.updateAge;
    lastAdded.age++;
    ...
    lastAdded.save(function(err) {
        ...
        return res.json({name:lastAdded.name, breed: lastAdded.breed, age: lastAdded.age});
    });
};
